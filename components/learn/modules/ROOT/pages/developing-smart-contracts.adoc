...
Once installed, we can run ``npx hardhat``. This will create an empty Hardhat project in our project directory.
...
[.hardhat]
--
Our `Box.sol` contract uses Solidity 0.6 so we need to first https://hardhat.org/config/#solc-configuration[configure Hardhat to use an appropriate solc version].

We specify a Solidity 0.6 solc version in our `hardhat.config.js`.
...
The https://hardhat.org/guides/compile-contracts.html#compiling-your-contracts[`compile`] built-in task will automatically look for all contracts in the `contracts` directory, and compile them using the Solidity compiler using the configuration in https://hardhat.org/config/#solc-configuration[`hardhat.config.js`].
