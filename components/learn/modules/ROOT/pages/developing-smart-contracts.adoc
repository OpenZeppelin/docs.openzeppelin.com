 ...
 * For a detailed guide that breaks down Solidity, https://www.alchemy.com/overviews/solidity-smart-contract/[ Alchemy's overview ] provides a high-level understanding of Ethereum smart contract components. This overview speaks to Solidity's mechanics, properties, syntax, and data types. 
...
[.truffle]
--
Our `Box.sol` contract uses Solidity 0.8 so we need to first https://truffleframework.com/docs/truffle/reference/configuration#compiler-configuration[configure Truffle to use an appropriate solc version].
