...
NOTE: It might feel like there are many moving pieces here, but each component has a well-defined role in building this application. That said, if you are new to the OpenZeppelin platform, it may help to look into the xref:contracts:gsn[OpenZeppelin Contracts GSN guide] and xref:on-dapps.adoc[how to build a dapp] before you continue reading.
...
NOTE: For most (d)apps, it is probably not a good idea to have such a generous policy since any malicious user can easily drain your contract from funds. Check out our https://example.com/[guide on GSN payment strategies] for different approaches to this problem.
