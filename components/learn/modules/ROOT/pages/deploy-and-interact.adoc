 ...
 * <<getting-started-with-the-cli, Install the OpenZeppelin CLI>>
...
You may have used the CLI already to xref:writing-smart-contracts.adoc#compiling-solidity-source-code[compile your contracts] - if not, install it now in a new npm project:
...
NOTE: You can call the OpenZeppelin CLI with both `openzeppelin` and `oz`. For brevity, from here on we will use the latter.
...
The command-line is useful for protyping and running one-off commands. However, eventually you will want to interact with your contracts from your own application, which means calling or sending transaction in code. In this section, we will see how to use https://web3js.readthedocs.io/en/1.2.4/[web3.js] to interact with our contracts from JavaScript, and https://github.com/OpenZeppelin/openzeppelin-contract-loader[@openzeppelin/contract-loader] to load them. Let's begin by installing them:
...
Let's begin coding in a new `src/index.js` file, where we will be writing our JavaScript script. We will start with some boilerplate for loading the libraries and https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function[writing async code].
...
We will first initialize a new `web3` instance. A `web3` instance is an object that holds a connection to a blockchain node, and allows us to send transactions and queries. We will create one connecting to the local development network we started earlier, that is running on localhost port 8545. 
...
In order to interact with the <<box-contract,`Box`>> contract we <<deploying-a-smart-contract, deployed using the CLI>>, we will create a new https://web3js.readthedocs.io/en/1.2.4/web3-eth-contract.html[web3 contract instance] using https://github.com/OpenZeppelin/openzeppelin-contract-loader[@openzeppelin/contract-loader]. 

A web3 contract instance is a javascript object that represents our contract in the blockchain, which we can use to interact with our contract. To create one, we need to provide the `contract-loader` with the contract name and the address where it was deployed - the address was the value returned by the CLI when we deployed the contract via `oz create`.
...
This snippet is equivalent to the `oz call` we ran earlier from the command line. Now, make sure everything is running smoothly by running the script again and checking the printed value:
...
NOTE: If you restarted ganache at any point, this script may fail. Restarting ganache clears all local blockchain state, so the `Box` contract instance will not be at the expected address.
