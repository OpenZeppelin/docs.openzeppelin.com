...
In the snippet above, just with the first two lines we get access to the Ethereum network via Metamask, falling back to Infura, as well as information on the network, provider, and even user accounts - all of this through the `useWeb3` hook provided by Network JS.
