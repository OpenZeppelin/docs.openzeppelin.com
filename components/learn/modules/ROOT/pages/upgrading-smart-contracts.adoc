...
We then need to configure Buidler to use our `@openzeppelin/buidler-upgrades` plugin. To do this add the plugin in your `hardhat.config.js` file as follows.
...
```console
$ npx hardhat run --network development scripts/deploy_upgradeable_box.js
...
```console
$ npx hardhat run --network development scripts/upgrade_box.js
All contracts have already been compiled, skipping compilation.
Box deployed to: 0xFF6049B87215476aBf744eaA3a476cBAd46fB1cA
```
--
