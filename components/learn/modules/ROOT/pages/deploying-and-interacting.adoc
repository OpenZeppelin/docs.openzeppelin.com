...
Hardhat Network will print out a list of available accounts and their private keys, along with its address, `http://127.0.0.1:8545`.
...
We use https://github.com/ethers-io/ethers.js[ethers] in our script, so we need to install it and the https://hardhat.org/plugins/nomiclabs-hardhat-ethers.html[hardhat-ethers plugin].
...
[.hardhat]
--
We need to add in our https://hardhat.org/config[configuration] that we are using the `hardhat-ethers` plugin.
