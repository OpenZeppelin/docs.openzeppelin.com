...
The risk pool has a unique ID and wallet in which the assets are managed. The risk pool can also issue its own risk pool tokens (ERC20), which can then be traded. The risk pool keeper can define how much must be deposited for collateral damage (collateralizatoinlevel1) and the maximum amount to which policies can be issued (sumOfSumInsuredCap). +
