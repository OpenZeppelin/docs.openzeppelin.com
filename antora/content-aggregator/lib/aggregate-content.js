...
function collectFilesFromStartPath (startPath, repo, authStatus, ref, worktreePath, originUrl, editUrl) {
  return (worktreePath
    ? readFilesFromWorktree(worktreePath, startPath, repo.dir)
    : readFilesFromGitTree(repo, ref, startPath)
  )
    .then((files) => {
...
async function readFilesFromWorktree (worktreePath, startPath, repoDir) {
...
async function readFilesFromGitTree (repo, ref, startPath) {
  const sha = await git.resolveRef(Object.assign({}, repo, { ref: ref.fullname }));
  const refDir = path.join(
    repo.gitdir.replace(/\.git$/, ''),
    makeHash(ref.shortname),
  );
  ...
  const files = await readFilesFromWorktree(checkoutDir, startPath, checkoutDir);
  await cleanUpGitTreeCache(checkoutDir, startPath);
  return files;
}
...
async function cleanUpGitTreeCache(checkoutDir, startPath) {
